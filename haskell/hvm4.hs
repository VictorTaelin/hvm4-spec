...
instance Show Term where
  ...
  show (Num n)       = show n
  show (Suc t)       = "+" ++ show t
  show (Sp0 t)       = "/Sp0(" ++ show t ++ ")"
  show (Sp1 t)       = "/Sp1(" ++ show t ++ ")"
...
parse_sp ps = do
  is_sp0 <- match "/Sp0" ps
  is_sp1 <- if is_sp0 then return False else match "/Sp1" ps
  case (is_sp0, is_sp1) of
    (True, _) -> do
      consume "(" ps
      t <- parse_term ps
      consume ")" ps
  ...
  case term of
    ...
    Sp0 t -> do
      ...
      case t' of
        Num n -> do
          inc_itrs e
          return (Num (((n * 16293) + 1) `mod` 0xFFFF))
        Sup l a b -> do
          inc_itrs e
    ...
    Sp1 t -> do
      ...
      case t' of
        Num n -> do
          inc_itrs e
          return (Num (((n * 32677) + 3) `mod` 0xFFFF))
        Sup l a b -> do
          inc_itrs e
