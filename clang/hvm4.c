...
fn Term Ddp(Term lab, Term val, Term body) {
  return New(DDP, 0, 3, (Term[]){lab, val, body});
}
...
fn Term Suc(Term x) {
  return New(SUC, 0, 1, (Term[]){x});
}

fn Term Sp0(Term x) {
  return New(SP0, 0, 1, (Term[]){x});
}

fn Term Sp1(Term x) {
  return New(SP1, 0, 1, (Term[]){x});
}
...
fn void str_term_go(Term term, u32 depth) {
  switch (tag(term)) {
    ...
    case NUM: {
      char buf[32];
      snprintf(buf, sizeof(buf), "%u", val(term));
      str_puts(buf);
      break;
    }
    case SUC: {
...
fn Term parse_term(PState *s, u32 depth) {
  ...
  if (match(s, "λ")) {
    ...
    t = Suc(parse_term(s, depth));
  } else if (match(s, "/")) {
    char tok[4] = {0};
    for (int i = 0; i < 3 && !at_end(s); i++) {
      tok[i] = peek(s);
      advance(s);
    }
    if (strcmp(tok, "Sp0") == 0) {
      ...
      consume(s, ")");
      t = Sp0(x);
    } else if (strcmp(tok, "Sp1") == 0) {
      consume(s, "(");
      Term x = parse_term(s, depth);
      consume(s, ")");
      t = Sp1(x);
    } else {
      parse_error(s, "Sp0 or Sp1", tok[0]);
    }
        ...
        case DSP: {
          ...
          switch (tag(whnf)) {
            ...
            case SUP: {
              ...
              u32  lab    = ext(whnf);
              u64  labloc = val(whnf);
              Term x      = HEAP[labloc + 0];
              Term y      = HEAP[labloc + 1];
              Copy A      = clone(lab, a);
              Copy B      = clone(lab, b);
              whnf        = Sup(lab, Dsp(x, A.k0, B.k0), Dsp(y, A.k1, B.k1));
              next        = whnf;
              goto enter;
            }
            case ERA: {
        ...
        case DDP: {
          ...
          switch (tag(whnf)) {
            ...
            case SUP: {
              ...
              Copy V      = clone(lab, v);
              Copy T      = clone(lab, t);
              whnf        = Sup(lab, Ddp(x, V.k0, T.k0), Ddp(y, V.k1, T.k1));
              next        = whnf;
              goto enter;
            }
            case ERA: {
              ITRS++;
              whnf = App(App(t, Era()), Era());
              next = whnf;
              goto enter;
            }
            default: {
        ...
        case SUC: {
          ...
          switch (tag(x)) {
            ...
            case SUP: {
              ...
              u32  lab    = ext(x);
              u64  labloc = val(x);
              Term a      = HEAP[labloc + 0];
              Term b      = HEAP[labloc + 1];
              whnf        = Sup(lab, Suc(a), Suc(b));
              next        = whnf;
              goto enter;
            }
            case ERA: {
...
fn Term collapse(Term term) {
  ...
  switch (tag(term)) {
    ...
    case DSP: {
      u64  loc = val(term);
      Term l   = collapse(HEAP[loc + 0]);
      Term a   = collapse(HEAP[loc + 1]);
      Term b   = collapse(HEAP[loc + 2]);
      return Dsp(l, a, b);
    }
    ...
    case DDP: {
      u64  loc = val(term);
      Term l   = collapse(HEAP[loc + 0]);
      Term v   = collapse(HEAP[loc + 1]);
      Term t   = collapse(HEAP[loc + 2]);
      return Ddp(l, v, t);
    }
